...
describe("calculateEquipmentBonusesFromGear", () => {
  describe("with Dizana's quiver", () => {
    describe("with weapon using ammo slot", () => {
      test("applies bonus when charged", () => {
        ...
        const bonuses = calculateEquipmentBonusesFromGear(
          playerWithChargedQuiver,
          monster,
        );
  ...
  describe("with demonic pact prayer scaling", () => {
    test("adds half worn prayer bonus to melee strength", () => {
      ...
      const baseBonuses = calculateEquipmentBonusesFromGear(
        basePlayer,
        monster,
      );
      const pactBonuses = calculateEquipmentBonusesFromGear(
        pactPlayer,
        monster,
      );

      expect(pactBonuses.bonuses.prayer).toBe(baseBonuses.bonuses.prayer);
      expect(pactBonuses.bonuses.str - baseBonuses.bonuses.str).toBe(
        Math.trunc(baseBonuses.bonuses.prayer / 2),
      );
    });
    ...
    test("treats Eclipse atlatl as a thrown weapon for thrown-weapon pacts", () => {
      ...
      expect(pactBonuses.offensive.ranged - baseBonuses.offensive.ranged).toBe(
        60,
      );
      expect(
        pactBonuses.bonuses.ranged_str - baseBonuses.bonuses.ranged_str,
      ).toBe(Math.trunc(baseBonuses.bonuses.str * 0.8));
    });
  });
});
