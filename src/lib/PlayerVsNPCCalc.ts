...
export default class PlayerVsNPCCalc extends BaseCalc {
  ...
  public getNPCDefenceRoll(): number {
    ...
    const statBonus = this.trackAdd(
      ...
      defenceStyle ? defenceData.bonus : 0,
      64,
    );
  ...
  private getNpcDefenceRollForStyle(
    defenceStyle: CombatStyleType,
    rangedBonusOverride?: number,
  ): number {
    return this.getNpcDefenceRollData(defenceStyle, rangedBonusOverride)
      .defenceRoll;
  }

  private getNpcDefenceRollData(
    defenceStyle: CombatStyleType,
    rangedBonusOverride?: number,
  ): {
      level: number;
  ...
  private getMinionHitDistribution(): HitDistribution | null {
    ...
    const zamorakItemCount = Math.max(
      0,
      Math.min(
        this.player.leagues.six.minionZamorakItemCount,
        MINION_MAX_ZAMORAK_ITEMS,
      ),
    );
    ...
    const candidateDists = (['ranged', 'magic'] as const).map((style) => {
      ...
      const finalDist = new AttackDistribution([baseDist]).transform(
        minionCalc.applyNpcTransforms(style),
      ).singleHitsplat;
    ...
    this.track(
      DetailKey.LEAGUES_MINION_DEFENCE_ROLL,
      bestCandidate.defenceRoll,
    );
  ...
  private getMinionTransformCalc(
    ...
    const minionCalc = new PlayerVsNPCCalc(minionPlayer, this.monster, <
      InternalOpts
    >{
      ...this.opts,
  ...
  protected getMinionDelayedHits(): DelayedHit[] {
    ...
    return minionDist.withProbabilisticDelays(() => [
      [1.0, MINION_ATTACK_SPEED],
    ]);
  }
  ...
  private getPlayerMaxMeleeHit(): MinMax {
    ...
    if (this.player.leagues.six.effects.talent_distance_melee_minhit) {
      ...
      minHit = this.trackAdd(
        DetailKey.LEAGUES_MIN_HIT_DISTANCE_MELEE,
        minHit,
        minhitBonus,
      );
    }
    ...
    if (
      ...
      maxHit = this.trackFactor(
        DetailKey.LEAGUES_MULTI_HIT_STR_INCREASE,
        maxHit,
        [100 + strengthBonus, 100],
      );
    }
    ...
    if (
      ...
      maxHit = this.trackFactor(
        DetailKey.LEAGUES_BLINDBAG_DAMAGE_BONUS,
        maxHit,
        [100 + damageBonus, 100],
      );
    }
  ...
  private getPlayerMaxRangedAttackRoll() {
    ...
    for (const p of this.getCombatPrayers('factorAccuracy')) {
      ...
      if (this.player.leagues.six.effects.talent_buffed_ranged_prayers) {
        factor = [
          Math.trunc(((factor[0] - factor[1]) * 13) / 10) + factor[1],
          factor[1],
        ];
      }
      effectiveLevel = this.trackFactor(
  ...
  private getPlayerMaxMagicHit(): MinMax {
    ...
    if (spellement && spellement === weakness?.element) {
      const bonus = this.trackFactor(
        DetailKey.MAX_HIT_SPELLEMENT_BONUS,
        baseMax,
        [weakness.severity, 100],
      );
  ...
  private getEnemyProtectionPrayerDamageFactor(
    ...
    const penetration = Math.min(
      this.player.leagues.six.effects.talent_prayer_pen_all ?? 0,
      100,
    );
  ...
  private getPrayerRestoreUnitsPerTick(drainResistance: number): number {
    ...
    if (
      ...
      const restoreInterval = Math.max(
        1,
        15 - Math.trunc(this.player.bonuses.prayer / 7),
      );
  ...
  private getBowRepeatHitBonus(baseMaxHit: number): number {
    ...
    const currentStacks = Math.max(
      this.player.leagues.six.bowHitStacks ?? 0,
      0,
    );
  ...
  public getMaxAttackRoll() {
    ...
    if (this.player.leagues.six.effects.talent_all_style_accuracy) {
      ...
      atkRoll = this.trackFactor(
        DetailKey.LEAGUES_ALL_STYLE_ACCURACY,
        atkRoll,
        [100 + factor, 100],
      );
    }
  ...
  private getDistributionImpl(): AttackDistribution {
    ...
    if (this.wearing("King's barrage")) {
      npcDist = attackerDist.transform(
        (h) => {
          const iceSplat = new Hitsplat(Math.trunc(h.damage / 2), h.accurate);
          const rangedSplat = new Hitsplat(
            h.damage - iceSplat.damage,
            h.accurate,
          );
  ...
  private getAttackerDist(): AttackDistribution {
    ...
    if (
      ...
      if (effectChance >= 1) {
        ...
        tmp.addHit(
          new WeightedHit((1 - acc) * effectChance, [Hitsplat.INACCURATE]),
        );
    ...
    if (this.isUsingMeleeStyle() && this.wearing('Dual macuahuitl')) {
      ...
      this.neededEchoes = [
        [acc, min, Math.max(min, firstMax)],
        [acc, min, Math.max(min, secondMax)],
      ];
    }
    ...
    if (
      ...
      const bonusDamage = this.trackFactor(
        DetailKey.LEAGUES_EARTH_SPELL_DEFENCE_BONUS,
        defenceLevel,
        [1, 12],
      );
      dist = dist.transform(flatAddTransformer(bonusDamage), {
        transformInaccurate: false,
      });
    }
    ...
    if (
      ...
      const lightDist = HitDistribution.linear(
        acc,
        Math.min(min, lightMax),
        lightMax,
      );
    ...
    if (
      ...
      return new AttackDistribution([
        HitDistribution.single(1.0, [new Hitsplat(dist.getMax())]),
      ]);
    }
  ...
  private npcTransformCache: {
    [k in Exclude<CombatStyleType, null>]?: HitTransformer;
  } = {};
  ...
  applyNpcTransforms(styleType: CombatStyleType): HitTransformer {
    ...
    const transformer: HitTransformer = (hitsplat) => {
      ...
      for (const t of relevantEffects) {
        dist = dist.wideTransform(
          t[0],
          t.length > 1 ? t[1] : DEFAULT_TRANSFORM_OPTS,
        );
      }
  ...
  private applyLeaguesPostProcessing(
    npcDist: AttackDistribution,
  ): AttackDistribution {
    const acc = this.getHitChance();
    const [min, max] = this.getMinAndMax();
    const leagues = this.player.leagues.six;
    ...
    if (
      ...
      let blindbagDist = new HitDistribution([
        new WeightedHit(1 - chanceBlindbagProc, [Hitsplat.INACCURATE]),
      ]);
      const partialDists = leagues.blindbagWeapons.map((weapon) => {
        ...
        return subCalc
          .getDistribution()
          .singleHitsplat.scaleProbability(
            chanceBlindbagProc * chanceThisWeapon,
          ).hits;
      });
      ...
      for (let i = 1; i <= 3; i++) {
        const thisRecurse = blindbagDist.scaleProbability(
          chanceBlindbagProc ** i,
        );
        thisRecurse.addHit(
          new WeightedHit(1 - chanceBlindbagProc ** i, [Hitsplat.INACCURATE]),
        );
        recursiveBlindBag = recursiveBlindBag.zip(thisRecurse);
        recursiveBlindBag = recursiveBlindBag.cumulative();
      }
      this.trackDist(
        DetailKey.DIST_LEAGUES_BLINDBAG_RECURSIVE,
        recursiveBlindBag,
      );
    ...
    if ((rangedEcho || meleeEcho) && !this.opts.isEcho) {
      ...
      if (leagues.effects.talent_thrown_maxhit_echoes && isWearingThrown) {
        ...
        echoDist = new HitDistribution(
          echoDist.hits.flatMap((hit) => [
            new WeightedHit(
              hit.probability * (1 - effectChance),
              hit.hitsplats,
            ),
            new WeightedHit(hit.probability * effectChance, [
              hit.anyAccurate()
                ? new Hitsplat(maxEchoHit)
                : Hitsplat.INACCURATE,
            ]),
          ]),
        ).flatten();
      ...
      echoDist.addHit(
        new WeightedHit(1 - combinedEchoHappeningChance, [Hitsplat.INACCURATE]),
      );
      npcDist.addDist(echoDist);

      if (leagues.effects.talent_ranged_echo_cyclical) {
        const cyclicChance = this.track(
          DetailKey.LEAGUES_ECHO_CHANCE_CYCLICAL,
          0.5,
        );
        const echoDistCyclical = echoDist.scaleProbability(cyclicChance);
        echoDistCyclical.addHit(
          new WeightedHit(1 - cyclicChance, [Hitsplat.INACCURATE]),
        );
    ...
    if (this.wearing('Fang of the hound') && this.isUsingMeleeStyle()) {
      ...
      this.trackDist(
        DetailKey.DIST_LEAGUES_FLAMES_OF_CERBERUS,
        flamesOfCerberusDist,
      );
  ...
  public getTtk(): number | undefined {
    ...
    if (this.player.leagues.six.minionEnabled) {
      ...
      try {
        return (
          sum(
            Array.from(this.getTtkDistribution().entries()),
            ([ticks, probability]) => ticks * probability,
          ) * SECONDS_PER_TICK
        );
      } catch (error: unknown) {
        if (
  ...
  public getTtkDistribution(): Map<number, number> {
    ...
    if (minionHits.length > 0) {
      ...
      for (
        let tick = 1;
        tick <= iterMax && minionEpsilon >= TTK_DIST_EPSILON;
        tick++
      ) {
        ...
        for (const [hp, hpProb] of hps.entries()) {
          ...
          if (minionDue) {
            ...
            combinedDist = combinedDist
              ? combinedDist.zip(minionHitDist).cumulative()
              : minionHitDist;
          }
          ...
          for (const wh of combinedDist.hits) {
            ...
            if (newHp <= 0) {
              minionTtks.set(
                tick,
                (minionTtks.get(tick) || 0) + chanceOfAction,
              );
  ...
  private getBlindbagUniques(): number {
    ...
    const uniqueIds = new Set(
      this.player.leagues.six.blindbagWeapons.map((eq) => eq.id),
    ).size;
